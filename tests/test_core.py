...
from sister.tokenizers import SimpleTokenizer, Tokenizer
...
class MeanEmbeddingCase(TestCase):
    def setUp(self):
        ...
        embedding.get_word_vectors.side_effect = lambda words: np.random.rand(
            len(words), 300
        )

        self.sentence_embedding = MeanEmbedding(
            tokenizer=SimpleTokenizer(), word_embedder=embedding
        )
